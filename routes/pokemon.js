...
pokemon.post("/", async (req, res, next) =>{
    const { pok_name, pok_height, pok_weight, pok_base_experiece } = req.body;
    
    if(pok_name && pok_height && pok_weight && pok_base_experiece){
        let query = "INSERT INTO pokemon (pok_name, pok_height, pok_weight, pok_base_experience)";
        query += ` VALUES('${pok_name}', ${pok_height}, ${pok_weight}, ${pok_base_experiece})`;   
...
pokemon.patch("/:id([0-9]{1,3})", async (req, res, next) => {
    ...
    return req.status(500).json({code: 500, message: "campos incompletos "});;
});
...
pokemon.get('/:name([A-Za-z]+)', async (req, res, next) => {
    ...
    const pkmn = await db.query("SEECT * FROM pokemon WHERE pok_name='" + name + "';")
   if(pkmn.length > 0) {
    return res.status(200).json({code: 200, message: pkmn});
    } 
    return res.status(404).send({code: 404, message: "Pokémon no encontrado"});
